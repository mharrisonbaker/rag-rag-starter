...
export function lint(text: string, index: Index): Finding[] {
  ...
  if (long.length) {
    ...
    findings.push({ ruleId: 'sentence-length', title: 'Short sentences',
      message: ${long.length} sentence(s) exceed ~25 words. Split and front-load the key point.,
      severity: 'warn', refs: src })
  }
  ...
  if (passive > 3) {
    ...
    findings.push({ ruleId: 'active-voice', title: 'Use active voice',
      message: Detected several passive constructions. Prefer subjectâ€“verbâ€“object with â€œyouâ€ where possible.,
      severity: 'warn', refs: src })
  }
  if (/click here|read more|learn more/i.test(text)) {
    const src = nearest(index, 'links link text descriptive')
    findings.push({ ruleId: 'link-text', title: 'Make link text descriptive',
      message: Replace â€œclick here/read moreâ€ with meaningful link text that names the target.,
      severity: 'info', refs: src })
  }
  const jargon = /(utilize|facilitate|leverage|paradigm|robust|synergy)/i
  if (jargon.test(text)) {
    const src = nearest(index, 'plain words simple familiar')
    findings.push({ ruleId: 'plain-words', title: 'Prefer plain words',
